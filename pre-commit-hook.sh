...
INCLUDE_STRING=`git diff --name-only --cached --diff-filter=d | { grep .cs || true;} | sed ':a;N;$!ba;s/\n/;/g'`
...
if [[ "$OSTYPE" == "msys"* ]]; then
    ...
    ./.git/hooks/resharper/cleanupcode.exe -p="Built-in: Full Cleanup" $SOLUTION_FILE --include="$INCLUDE_STRING"
elif [[ "$OSTYPE" == "cygwin" ]]; then
    #Cygwin terminal emulator
    ./.git/hooks/resharper/cleanupcode.exe -p="Built-in: Full Cleanup" $SOLUTION_FILE --include="$INCLUDE_STRING"
else
    sh ./.git/hooks/resharper/cleanupcode.sh -p="Built-in: Full Cleanup" $SOLUTION_FILE --include="$INCLUDE_STRING"
fi
